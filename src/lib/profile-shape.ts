  ...
  return asArray(v)
    .map((item) => {
      ...
      return {
        ...
        employmentType: str(o.employmentType ?? o.type ?? o.employment_status),
        location: str(o.location),
        locationType: str(o.locationType ?? o.workArrangement ?? o.workMode),
...
export function normalizeParsedResume(raw: any) {
  ...
  return {
    personal: {
      ...
      preferredName: str(p.preferredName ?? p.preferred_name),
      email: str(p.email),
      phone: str(p.phone ?? p.phoneNumber),
      phoneCountryCode: str(p.phoneCountryCode ?? p.countryPhoneCode),
      phoneType: str(p.phoneType) || "Mobile",
      dateOfBirth: str(p.dateOfBirth ?? p.dob),
      address: str(p.address),
      addressLine2: str(p.addressLine2 ?? p.address2),
      city: str(p.city),
      state: str(p.state ?? p.region),
      zip: str(p.zip ?? p.postalCode ?? p.postcode),
    ...
    targetRole: str(raw?.targetRole ?? raw?.target_role),
    skills: toStringArray(raw?.skills),
    certifications: toStringArray(raw?.certifications ?? raw?.licenses),
    experience: toExperience(raw?.experience ?? raw?.workExperience ?? raw?.work_history),
    education: toEducation(raw?.education ?? raw?.academicHistory),
    websites: toWebsites(raw?.websites ?? raw?.links ?? raw?.profiles),
    workAuth: {
      authorizedToWork: str(raw?.workAuth?.authorizedToWork),
      requireSponsorship: str(raw?.workAuth?.requireSponsorship),
      workAuthType: str(raw?.workAuth?.workAuthType),
      ...
      remotePreference: str(raw?.workAuth?.remotePreference),
    },
    compensation: {
