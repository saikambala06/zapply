...
const MAX_BYTES = 3 * 1024 * 1024; // Keep the stored base64 payload safely below Vercel/Mongo document limits
...
export const POST = handler(async (req: Request) => {
  ...
  if (file.size > MAX_BYTES) return fail("That resume is over 3 MB. Please export/compress it to a smaller PDF or DOCX and try again.", 413);
  ...
  // BSON document limit. A 3 MB PDF becomes ~4 MB as base64 and is still safe,
